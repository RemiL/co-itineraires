...
import observateurs.UsagerAtteintArrivee;
import observateurs.UsagerQuitteLieu;
import observateurs.VehiculeQuitteStation;
...
public class TestSimulateur 
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			usagers.add(new Usager("Jane Doe", lieux.get("Batiment 337 - Salles d'examen"), lieux.get("Arrêt L'Yvette (retour)"), trajets, contraintes.get(1), 1));
			...
			usagers.add(new Usager("Jane Smith", lieux.get("Batiment 337 - Salles d'examen"), lieux.get("Arrêt L'Yvette (retour)"), trajets, contraintes.get(2), 2));
			contraintes.add(new EviterMoyenTransport(PrendreBus.getInstance()));
			usagers.add(new Usager("John Martin", lieux.get("Batiment 337 - Salles d'examen"), lieux.get("Arrêt L'Yvette (retour)"), trajets, contraintes.get(3), 3));
			...
			Observer usagerQuitteLieu = new UsagerQuitteLieu(lieux.get("Arrêt Bures Amphi (retour)")); // new UsagerQuitteLieu()
			Observer usagerAtteintArrivee = new UsagerAtteintArrivee();
			Observer vehiculeQuitteStation = new VehiculeQuitteStation(lieux.get("Arrêt Bures Amphi (retour)"));
			
			for (Usager u : usagers)
			{
				u.addObserver(usagerQuitteLieu);
				u.addObserver(usagerAtteintArrivee);
			}
			
			for (Ligne l : lignesBus.values())
			{
				l.addObserver(vehiculeQuitteStation);
			}
			
			ArrayList<Simulable> objetsAFaireEvoluer = new ArrayList<Simulable>(lignesBus.values());
			SimulateurDeplacement simulateur = new SimulateurDeplacement(usagers, objetsAFaireEvoluer);
	...
	private static void initialiserLieux()
	{
		...
		try
		{
			...
			int[] horairesTests = {0,15,30,45,60,75,90,105,120,135,150,165,180,195};
			
			// Ligne de bus 06-07 Vallée
			lignesBus.put("06-07 Vallée", new Ligne("06-07 Vallée", PrendreBus.getInstance(), horairesTests));
			...
			new Etape(lieux.get("Arrêt Jean Monnet (aller)"), lieux.get("Arrêt Jean Monnet (retour)"), PrendreBus.getInstance(), lignesBus.get("06-07 Vallée"), 0, 10);
			...
			lignesBus.put("06-07 Plateau", new Ligne("06-07 Plateau", PrendreBus.getInstance(), horairesTests));
			...
			new Etape(lieux.get("Arrêt Corbeville (aller)"), lieux.get("Arrêt Corbeville (retour)"), PrendreBus.getInstance(), lignesBus.get("06-07 Plateau"), 0, 10);
